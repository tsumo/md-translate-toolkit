...
Each entry has a number, so other text can refer to it (for example "see ADR-006"). When a decision changes,
...
### ADR-003: One translation file per document
...
exception. Each placeholder in a translation file holds a short preview of its original text (ADR-006).

### ADR-005: Files are linked through the manifest
...
### ADR-006: Translation skeleton with placeholders
...
from the block status (ADR-010).

### ADR-007: Translators edit in their own editor
...
### ADR-008: Applying updates changes translations
...
### ADR-009: Structure drift fails the build
...
### ADR-010: Block status is stored, document status is derived
...
### ADR-011: Block fingerprints ignore formatting
...
### ADR-012: Upstream renames, deletions, and splits need a person
...
### ADR-013: Diff with a standard library algorithm
...
removed block loses its translation (ADR-008).

### ADR-014: Scripts and utility modules
...
### ADR-015: Command line only, configured by a file
